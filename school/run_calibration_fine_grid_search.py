...
for k, sample_index in enumerate(samples):
    ...
    results.to_csv(join(dst, 'calibration_results_{}_samples{}_runs_{}_fine_grid_{}-{}_curr.csv'\
		.format(school_type, len(samples), N_runs, N_low, N_high)), index=False)
    
results.to_csv(join(dst, 'calibration_results_{}_samples{}_runs{}_fine_grid_{}-{}.csv'\
		.format(school_type, len(samples), N_runs, N_low, N_high)), index=False)