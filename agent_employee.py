...
class Employee(Agent):
    ...
    def step(self):
        ...
        if self.infected:
            ...
            patients = [a for a in self.model.schuedule.agents if a.type == 'patient']
            # get a list of employees
            employees = [a for a in self.model.schuedule.agents if a.type == 'employee']

            # code transmission to employees and transmission to patients separately
            # to allow for differences in transmissions pater
            for p in patients:
                if (p.exposed == False) and (p.infected == False) and \
                   (p.recovered == False) and (p.contact_to_infected == False):
                    ...
                    if transmission <= self.model.transmissoin_risk_employee_patient:
                ...
                if (e.exposed == False) and (e.infected == False) and\
                   (e.recovered == False) and (e.contact_to_infected == False):
                    ...
                    if self.verbose > 1: 
                        ...
                        print('tranmission prob {}'.format(transmission))
                    if transmission <= self.model.transmissoin_risk_employee_employee:
